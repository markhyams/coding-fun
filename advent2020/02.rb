...
def num_valid_passwords(input)
  result = 0
  input.split(/\n/).each do |line|
    record = password_line_to_object(line)
    result += 1 if record_valid?(record)
  end

  result
end

def num_valid_passwords_2(input)
  result = 0
  input.split(/\n/).each do |line|
    record = password_line_to_object(line)
    result += 1 if record_valid_2?(record)
  end

  result
end

def password_line_to_object(str)
  range, letter, password = str.split
...
def record_valid?(obj)
  count = obj[:password].count(obj[:letter])
  count >= obj[:lower] && count <= obj[:upper]
end

def record_valid_2?(obj)
  idx1 = obj[:lower] - 1
  idx2 = obj[:upper] - 1
  word = obj[:password]
  [word[idx1], word[idx2]].count(obj[:letter]) == 1
end

str1 = "1-3 a: abcde" # is valid: position 1 contains a and position 3 does not.
str2 = "1-3 b: cdefg" # is invalid: neither position 1 nor position 3 contains b.
str3 = "2-9 c: ccccccccc" # is invalid: both position 2 and position 9 contain c.

p record_valid_2?(password_line_to_object(str1)) == true
p record_valid_2?(password_line_to_object(str2)) == false
p record_valid_2?(password_line_to_object(str3)) == false

p num_valid_passwords(input) == 456
p num_valid_passwords_2(input)
